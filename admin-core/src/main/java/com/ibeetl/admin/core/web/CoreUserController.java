...
public class CoreUserController {
	...
	@ResponseBody
	public JsonResult<UserLoginInfo> login(String code, String password, HttpServletRequest request) {
		UserLoginInfo info = userService.login(code, password);
		if (info == null) {
			
			return JsonResult.failMessage("用戶名密碼錯");
		}
		...
		session.setAttribute(CorePlatformService.ACCESS_CURRENT_USER,info.getUser());
		session.setAttribute(CorePlatformService.ACCESS_CURRENT_ORG,info.getCurrentOrg());
		session.setAttribute(CorePlatformService.ACCESS_USER_ORGS,info.getOrgs());
		session.setAttribute("ip", request.getRemoteHost());
		this.httpRequestLocal.set(request);
//		this.platformService.setLoginUser(info.getUser(), info.getCurrentOrg(), info.getOrgs());
	...
	public JsonResult login(Long orgId) {
		...
		if (currentOrg == null) {
			// 非法切換
			return JsonResult.failMessage("切換到不存在的部門");
		}
	...
	public JsonResult chnagePassword(String password, String newPassword) {
		...
		if (realUser.getPassword().equals(pwd)) {
			...
			return JsonResult.failMessage("密碼錯誤");
		}
