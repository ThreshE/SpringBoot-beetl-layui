...
public class CoreCodeGenController {
    ...
    public ModelAndView project() {
        ...
        view.addObject("basePackage","com.castlers.xxx");
        return view;
      
    }

	@GetMapping(MODEL + "/projectDownload.do")
	public void projectDownload(String path, HttpServletResponse response) {
		String replacePath = path.replace("\\", "/");
		String zipPath = replacePath + ".zip";
		String zipFileName = zipPath.substring(zipPath.lastIndexOf("/") + 1);
		try {
			generateZipFile(replacePath);
			...
			log.error("下載項目失敗：" + e);
			throw new RuntimeException("下載項目失敗：" + e.getMessage());
		}

	}

	private void generateZipFile(String replacePath) throws ZipException {
		ZipFile zipFile = new ZipFile(replacePath + ".zip");
    ...
    public JsonResult project(String path,String basePackage,String includeConsole) throws IOException {
        //includeConsole 當前版本忽略，總是添加一個係統管理功能，可以在pom中移除console
	...
	public JsonResult gen(EntityInfo data,String path) {
		...
		//生成到path目錄下，按照maven專案解構生成
		File file = new File(path);
		if(!file.exists()) {
		    throw new PlatformException("路徑不存在 "+path);
		}
		...
		MdGen mdGen = new MdGen(basicFunctionCode);
	...
	public JsonResult<Map<String, String>> sql(EntityInfo data) {
		...
		WebTarget webTarget = new WebTarget(entity, basePackage);
		webTarget.setUrlBase(urlBase);
		MdGen javaGen = new MdGen(basicFunctionCode);
		javaGen.make(webTarget, entity);
		Map<String, String> content = new HashMap<String, String>();
		for (Entry<Object, String> entry : webTarget.map.entrySet()) {
			AutoGen gen = (AutoGen) entry.getKey();
	...
	private Entity getEntitiyInfo(EntityInfo data) {
		...
		for (int i = 0; i < entity.getList().size(); i++) {
		    ...
		    attr.setDisplayInTable(info.getList().get(i).isDisplayInTable());
		...
		if (StringUtils.isEmpty(entity.getCode()) || StringUtils.isEmpty(entity.getSystem())) {
			throw new PlatformException("code,system不能為空");
		}
	...
	public void test(@PathVariable String table) {
		...
		createTimeAttr.setDisplayName("創建日期");
		...
		createUserIdAttr.setDisplayName("創建人");
