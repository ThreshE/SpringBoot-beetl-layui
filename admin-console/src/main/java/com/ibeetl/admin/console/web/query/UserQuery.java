...
public class UserQuery extends PageParam {
	...
	private String name ;
	@Query(name="機構",display=true,type=Query.TYPE_CONTROL,control="org")
	private Long orgId;

	@Query(name="狀態",display=true,type=Query.TYPE_DICT,dict=CoreDictType.USER_STATE)
	private String state;

	@Query(name="職位",display=true,type=Query.TYPE_DICT,dict="job_type",group="job_type")
	private String jobType0;

	@Query(name="職位明細",display=true,type=Query.TYPE_DICT,dict="",group="job_type")
